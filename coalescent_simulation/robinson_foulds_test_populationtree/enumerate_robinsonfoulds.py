...
# Run like:
# python3 enumerate_robinsonfoulds.py ASTRAL_90strip_noP7_WH12.rooted.scaleX2.simulation.tre out_simulated.csv RAxML_bipartitions.concatenated_nohybrid_90stripped_bwa.rooted.samplingmatched.tre out_empirical.csv
...
cp_tree = dendropy.TreeList.get_from_string(cp_tree_str, "newick", preserve_underscores = True, suppress_internal_node_taxa = True, suppress_leaf_node_taxa = False, taxon_namespace=tns)

print(len(cp_tree))

print("Generating empirical distances.\n")
output = []
for tree1 in sp_tree:
	for tree2 in cp_tree:
		b_cp = tree2.encode_bipartitions()
		output.append(treecompare.symmetric_difference(tree1, tree2))
